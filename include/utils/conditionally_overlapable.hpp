...
LEV_HIDDEN inline constexpr bool fits_in_tail_padding_v = []() {
    struct test_struct {
        LEV_NO_UNIQUE_ADDRESS T0 first;
        LEV_NO_UNIQUE_ADDRESS T1 second;
    };
...
struct conditionally_overlapable {
    LEV_HIDE_INSTANTIATION inline constexpr conditionally_overlapable() = delete;
    ...
    LEV_HIDE_INSTANTIATION inline constexpr explicit conditionally_overlapable(std::in_place_t,
        Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args...>)
        : data{std::forward<Args>(args)...} {}

    template <typename F, typename... Args>
    LEV_HIDE_INSTANTIATION inline constexpr explicit conditionally_overlapable(generate_t, F&& f,
